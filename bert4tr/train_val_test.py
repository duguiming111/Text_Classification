# Author: dgm
# Description: 训练、验证、测试
# Date: 2020-11-06
import time
import torch
import numpy as np
from sklearn import metrics
from bert4tr.utils import get_time_dif
...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            loss = loss_fct(outputs.view(-1, config.num_classes), labels.view(-1))
            loss.backward()
            optimizer.step()
            scheduler.step()
...
def test(config, model, test_iter):
    # test
    model.load_state_dict(torch.load(config.best_model_dir))
    model.eval()
    start_time = time.time()
